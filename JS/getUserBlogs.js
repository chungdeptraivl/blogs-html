...
async function getHomeBlogs(token) {
  try {
    ...
    const response = await fetch(
      `http://localhost:5105/api/Post/AllPublished?AuthorId=${authorId}`,
      {
        method: "GET",
        headers: {
...
async function contentPost() {
  ...
  document.getElementById('userName').innerText = postData[0].author.username
  document.getElementById('userEmail').innerText = postData[0].author.email
